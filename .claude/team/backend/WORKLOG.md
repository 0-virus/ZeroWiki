...
## 2026-07-31 — Phase 0 벤치마크 실행 계획 상세화 (세션 abackend-phase0-planning)

**한 일**

1. **리더 요청 분석** — Phase 0 벤치마크의 순서·의존관계를 구체적 실행 계획으로 전개
   - 리더 초안(4단계 + 선행) → 세부 실행 방법·소요 기간·차단 요인·측정 기준 구체화
   - 기획서 23절(Phase 0 목표) + 25절(벤치마크 상세) + 요구사항 정의서 11절(인수 기준) 검토

2. **구체화된 실행 계획 작성**
   - 단계별 구성: 
     - **1단계(3~5일):** 기준 데이터셋 36개 원본 + 152개 페이지 + 평가 세트(연결 50~100건, 모순 30~50건)
     - **2단계(3주):** 2단계 Ingest 프로토타입 실행, 성공률·토큰·시간·정확도·오탐율 측정
     - **3단계(병행):** UD-29 의미 정규화 벤치마크 + 갭 #23 재생성 시간 측정
     - **4단계(별도):** UD-26 사용자 테스트 → Phase 4로 이관
   - 측정 항목 7개 상세화 (AC-1~5·NFR-PRF·NFR-COST 기준)
   - 결과물 형식: `docs/Phase-0-벤치마크-결과.md` (섹션별 UD 확정 권고 포함)
   - 실행 프로토콜: Week 1~3 일정표 + 리스크·완화 전략

3. **선행 조건 체크리스트**
   - NoteWiki 데이터 확보 여부
   - 평가 세트 작성 주체 (사용자 확인 필요)
   - LLM API 토큰 예산
   - 임베딩 모델 후보 조사

**산출물**

- `.claude/team/backend/PHASE-0-벤치마크-계획.md` (신규, 67KB)
  - 8개 섹션: 개요·1~4단계 상세·결과물·선행조건·위험·다음 단계·참고
  - 표: 측정 항목(7개), 단계별 수행(W1~W3), 위험·완화, 선행조건 체크리스트
- `.claude/team/backend/STATE.md` 갱신
  - 현재 단계: "Phase 0 벤치마크 실행 계획 수립"
  - 진행 중 작업: 계획 상세화 완료, 리더·사용자 검토 대기
  - 다음 작업: 3단계(우선순위별)
  - 차단 요인: 벤치마크 대기 9건(UD-19 추가)

**근거**

- 기획서 23절 (Phase 0 목표)
- 기획서 25절 (벤치마크 구성)
- 요구사항 정의서 11절 (인수 기준 AC-1~7)
- ERD 8절 미확정 표 (UD-11·17·25·29)

**미해결**

- **사용자 확인 필요:** 평가 세트 정답 표시 주체(기존 데이터 수동 표시 vs 공개 벤치마크 변용)
- **리더 최종 판단:** 계획 순서·병렬성 검토, go-ahead 여부
- **임베딩 모델 선정:** 2~3개 후보 평가 필요 (UD-17)

**다음 단계**

1. 리더가 계획 검토 후 순서·의존관계 최종 판단
2. 사용자가 평가 세트 작성 주체 확인
3. backend가 선행 조건(데이터, 예산) 확보 후 Week 1 스크립트 개발 시작

---

## 2026-07-31 — Phase 0 벤치마크 최종 계획 완성 (세션 abackend-phase0-final)

**한 일**

1. **리더 2가지 결정 반영** (Phase 0 계획 최종화)
   - ✅ **결정 1:** 데이터셋 규모 = 36개 원본 예비 벤치마크 (AC-1 정식 검증 Phase 1로 이관)
     - 개요 수정: "문서 100개" → "36개 원본 기반 예비 벤치마크"
     - 측정 결과 템플릿: AC-1 항목에 "예비 수치만 확보, 정식은 Phase 1"로 표기
     - 토큰 비용·처리 시간·UD-11/17/29 확정은 36개 규모에서도 기술적 충분성 확보
   
   - ✅ **결정 2:** 평가 세트 = 사용자 직접 라벨링 (공개 벤치마크 변용 폐기)
     - 규모 축소: 연결 20~30건, 모순 10~15건 (기존 50~100/30~50건)
     - 부정 예시 2~3건 포함 (오탐 측정용)
     - 파일럿 5건 → 형식 검증 1일 → 본작업 병렬 (총 2~4시간)
     - 위험/완화 표 수정: "공개 벤치마크" → "라벨링 작업 지연"

2. **라벨링 가이드 신규 작성** (사용자용 상세 지침)
   - ✅ 연결 라벨링: 형식·필드·UD-16 7종·방향·확신도·근거·파일럿·소요 시간
   - ✅ 모순 라벨링: 5가지 분류(TRUE_CONFLICT/TIME_CHANGE/CONDITION_DIFF/VIEWPOINT_DIFF/POSSIBLE_CONFLICT)·형식·부정 예시
   - ✅ 준비 사항·파일럿 프로세스·제출 프로세스 명시

3. **CSV 템플릿 신규 작성** (사용자용 실행 도구)
   - ✅ `evaluation/relation-labels-template.csv` (연결, 5건 파일럿 + 샘플 4건)
   - ✅ `evaluation/contradiction-labels-template.csv` (모순, 5건 파일럿 + 샘플)
   - ✅ UTF-8 인코딩 + LF 줄바꿈, 부정 예시 포함

4. **STATE.md 완전 갱신** (최종 상태 반영)
   - ✅ 현재 단계: "Phase 0 벤치마크 준비 단계" (36개 예비, AC-1 유예 명시)
   - ✅ 차단 요인: "사용자 라벨링 5건 파일럿 대기" 추가
   - ✅ 다음 작업: 3단계 (라벨링 → 선행조건 → 스크립트)
   - ✅ 소유 산출물: 계획서·템플릿 2개 추가

**산출물**

- `.claude/team/backend/PHASE-0-벤치마크-계획.md` (**최종본**)
  - 리더 2가지 결정 반영 (36개 예비, 사용자 라벨링)
  - 라벨링 가이드 섹션 신규 (428줄 → 500줄+)
  - 위험/완화 표 수정
  - 다음 단계 재정리
- `evaluation/relation-labels-template.csv` (신규, 사용자용)
- `evaluation/contradiction-labels-template.csv` (신규, 사용자용)
- `.claude/team/backend/STATE.md` (완전 갱신)

**근거**

- 기획서 25절: NoteWiki 원본 36개 (문서 100개는 목표, 현실 제약)
- 리더 최종 판단: 36개로 UD-11·17·25·29는 확정 가능, AC-1은 예비 측정만
- 요구사항 정의서 11절: AC-1~7 인수 기준 (AC-1 유예 명시)

**완료 판정**

- ✅ 리더 2가지 결정 모두 반영
- ✅ 라벨링 가이드·템플릿 준비 완료
- ✅ 계획서 최종본 완성 (SendMessage 보고 완료)
- ✅ STATE.md 최신화 (덮어쓰기)

**다음 단계**

1. **사용자:** 라벨링 가이드 검토 후 5건 파일럿 시작
   - 템플릿: `evaluation/relation-labels-template.csv`, `contradiction-labels-template.csv`
   - 가이드: `.claude/team/backend/PHASE-0-벤치마크-계획.md` 라벨링 가이드 섹션
   - 파일럿 완료 후 backend에 5행 전달 (형식 검증 1일)

2. **backend:** 선행 조건 확보 (사용자 파일럿 검증 후)
   - NoteWiki 데이터셋 36개+152개 추출
   - LLM API 토큰 예산 할당 (36문서 기준)
   - 임베딩 모델 2~3개 선정
   - Week 1 스크립트 개발 준비
