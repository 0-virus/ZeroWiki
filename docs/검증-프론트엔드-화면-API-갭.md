...
없음. 모든 기존 갭은 2026-07-29 변경 이후에도 여전히 유효하다.
...
**상태**: **잔존** (API 명세 선택사항 불명확)  
**영향 화면**: 도서관 홈  
**현재 상황** (API 명세 4.1절): `GET /libraries/{libraryId}`가 "다음을 포함할 수 있다" 방식으로 선택사항 명시. 클라이언트 입장에서 응답에 `topicSummary`, `recentRelations` 등이 있는지 없는지 불명확.  
...
**상태**: **잔존** (기획서 미명확)  
**영향 화면**: 도서관 홈 (요소 1)  
**현재 상황** (기획서 16.2절): "주요 주제"라고 표현하지만, 구체적으로 무엇을 보여줄지 불명확. AI가 자동 생성한 개념? 상위 분류? 사용자가 정의한 카테고리?  
**문제**: UI 설계와 API 설계가 엇갈릴 수 있음.  
**심각도**: **MAJOR**  
**근거**: 요구사항 정의서 2절 참조 (기획서 기준 문서)
...
**현재 상황** (API 명세 18절 #5): "unified diff vs 구조화 블록 diff" 미결정. 현재 스키마는 `beforeSnapshot`, `afterSnapshot` 전체 `jsonb` 반환.  
**문제**: 페이지 본문(markdown)의 diff를 어떻게 표현할지 불명확.  
**심각도**: **BLOCKER**  
**근거**: 요구사항 정의서 12.1절 UD-05 (기술 결정, 미확정)

**현재 설계**:
- API 명세 8.1절: `beforeSnapshot`, `afterSnapshot` 전체 객체 반환
- 클라이언트가 diff 라이브러리로 계산 (Phase 1)
...
**현재 상황** (API 명세 18절 #4): "MVP는 polling 기본"이라고만 하고, 구체적 간격은 미확정.  
**문제**: 간격이 너무 짧으면 서버 부하, 너무 길면 UX 지연.  
**심각도**: **MAJOR**  
**근거**: 요구사항 정의서 12.1절 UD-04 (기술 결정, 미확정)

**권고** (작업 #2에서 확정 필요):
- Ingest (진행도 있음): 2~3초
- AI 답변 (생성 중): 1초 또는 즉시 재조회
- Lint·Export: 2~5초
...
### 갭 #16: 원문 용어 병기 렌더링과 검색

**상태**: **신규** (API 명세 2.8절 전문 용어 병기 규칙 추가, UD-27 확정)  
**영향 화면**: 모든 위키 본문, 변경 검토 diff  
**변경 사항**: 페이지 본문에 `한국어(원어)` 형식의 병기가 나타남.
- **규칙** (요구사항 정의서 5.15절 FR-UIX-14):
...
**UI 영향**:
- Markdown 렌더링이 괄호 안 텍스트를 올바르게 표시해야 함 (굵게? 이탤릭? 보통?)
- 검색 기능:
  - 사용자가 "attention mechanism" 검색 → 병기된 페이지 히트되어야 함 (괄호 안 텍스트 포함 인덱싱)
  - 사용자가 "주의 기제" 검색 → 양쪽 다 히트
- 변경 검토 diff에서 병기 추가/제거가 "내용 변경"으로 오인되지 않도록 주의
