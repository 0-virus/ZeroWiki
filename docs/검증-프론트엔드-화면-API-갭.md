...
### 갭 #2: 도서관 홈 요약의 `topics` 산출 기준

**상태**: **해소됨** (UD-28 확정, 2026-07-29)  
**영향 화면**: 도서관 홈  
**확정 내용** (2026-07-29): 
- **주제 정의:** `page_type='CONCEPT'` 페이지
- **측정 방법:** `ACCEPTED` 관계 수의 양방향 합산
- **표시 개수:** 상위 6개
- **대상:** `PUBLISHED` 페이지만
- **재계산 시점:** 변경 세트 apply 커밋 후 비동기 배치
**근거**: 요구사항 정의서 0.8절 개정 이력, API 명세 4.1절 427행
...
**상태**: **해소됨** (UD-28 확정, 2026-07-29)  
**영향 화면**: 도서관 홈 (요소 1)  
**확정 내용**: 갭 #2 동일. `CONCEPT` 페이지 중 `ACCEPTED` 관계 수 양방향 합산 기준으로 상위 6개 `PUBLISHED` 페이지를 주요 주제로 표시.  
**근거**: 요구사항 정의서 0.8절 개정 이력, 기획서 16절, FR-UIX-15 확정
...
### 갭 #19: 업로드 파일 크기·개수 한도 검증 및 413 에러 처리

**상태**: **신규** (UD-01 확정, 2026-07-30)  
**영향 화면**: 파일 업로드 화면  
**확정 내용** (UD-01, 2026-07-30):
- 파일당 최대 200MB
- ZIP당 최대 1GB
- Import당 최대 1000개 문서
- 초과 시 HTTP 413 (Payload Too Large) 응답

**UI 영향**:
- **사전 검증**: 파일 선택 시 클라이언트 측 크기 체크 (200MB/파일, 1GB/ZIP)
- **개수 안내**: "최대 1000개 문서까지 한 번에 업로드 가능"
- **413 에러 표시**: 서버 거절 시 "파일이 너무 크거나 문서 개수가 초과되었습니다. 세션을 분할해서 다시 시도하세요."
- **업로드 진행 중 크기 누적 표시** (ZIP 압축해제 후)

**근거**: 요구사항 정의서 12.1절 UD-01 (확정, 2026-07-30), API 명세 2.3절 (413 응답)

---

### 갭 #20: 토큰 만료 시 세션 재발급 UX (비동기 작업 진행 중)

**상태**: **신규** (UD-02 확정, 2026-07-30)  
**영향 화면**: 모든 비동기 작업 진행 화면 (Ingest, AI 답변, Lint, Export)  
**확정 내용** (UD-02, 2026-07-30):
- Access Token 수명: **15분**
- Refresh Token 수명: **30일**
- 15분은 매우 짧으므로, 대량 Ingest나 복잡한 AI 답변 생성 중 토큰 만료 가능

**UI 영향**:
- **polling 중 401 응답 처리**: 토큰 만료 감지 시 투명하게 재발급 (Refresh Token 사용)
- **재발급 실패 시**: 사용자에게 알림 ("세션이 만료되었습니다. 다시 로그인하세요")
- **진행 중인 작업 상태 유지**: 토큰 재발급 후 같은 jobId/messageId로 polling 재개
- **오프라인/네트워크 재연결 시**: 토큰이 없으면 로그인 화면으로 리다이렉트

**구현 고려사항**:
- axios/fetch 인터셉터에서 401 → Refresh Token으로 재시도 자동화
- Refresh Token도 만료되면 로그인 필요
- 다중 탭 열림 상태에서 한 탭이 토큰을 갱신해도 다른 탭에서는 401이 발생 → 로컬 스토리지 이벤트 또는 BroadcastChannel로 동기화

**근거**: 요구사항 정의서 12.1절 UD-02 (확정, 2026-07-30), API 명세 2.1절 (인증 정책)
